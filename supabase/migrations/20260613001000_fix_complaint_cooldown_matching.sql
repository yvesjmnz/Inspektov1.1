...
CREATE OR REPLACE FUNCTION public.complaint_cooldown_establishment_matches(
  p_existing_business_pk integer,
  p_existing_business_name text,
  p_existing_business_address text,
...
AS $$
  SELECT
    (
      p_business_pk IS NOT NULL
      AND p_existing_business_pk = p_business_pk
    )
    OR
    (
      public.normalize_complaint_establishment_text(p_business_name) <> ''
      AND public.normalize_complaint_establishment_text(p_business_address) <> ''
      AND public.normalize_complaint_establishment_text(p_existing_business_name)
        = public.normalize_complaint_establishment_text(p_business_name)
      AND public.normalize_complaint_establishment_text(p_existing_business_address)
        = public.normalize_complaint_establishment_text(p_business_address)
    );
$$;
...
BEGIN
  ...
  WHERE lower(coalesce(mo.status, '')) NOT IN ('cancelled', 'canceled')
    AND public.complaint_cooldown_establishment_matches(
      c.business_pk,
      c.business_name,
      c.business_address,
      p_business_pk,
